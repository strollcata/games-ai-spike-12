...
box_kind_color = {
    ...
    'I': (1.0, 1.0, 1.0, 1.0), # item, White
    'P': (1.0, 1.0, 1.0, 1.0)  # point, White
}
...
class Box(object):
    ...
    def draw(self):
        ...
        if self.kind == 'I':
            ...
            egi.line(self._pts[0].x + 10, self._pts[0].y - 5, self._pts[0].x + 10, self._pts[0].y - 16)
            egi.line(self._pts[0].x + 5, self._pts[0].y - 11, self._pts[0].x + 16, self._pts[0].y - 11)

        if self.kind == 'P':
            obj_pts = (
                Point2D(self._pts[0].x + 5, self._pts[0].y - 5), # top left
                Point2D(self._pts[0].x + 15, self._pts[0].y - 4), # top right
                Point2D(self._pts[0].x + 15, self._pts[0].y - 15), # bottom right
...
#            # DOWN RIGHT (i - nx + 1)
    ...
    def plan_path(self, search, limit):
        ...
        '''
        cls = SEARCHES[search]
        self.path = cls(self.graph, self.start.idx, self.target.idx, self.item_idx, self.point_idx, limit)
    ...
    def items_and_points(self):
        ...
        box_selected = next((box for box in self.boxes if box.pos == box_change.pos), None)
        box_selected.kind = 'I'
